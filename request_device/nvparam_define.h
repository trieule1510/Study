 ...
 */
#ifndef __NVPARAM_DEFINE_H__
#define __NVPARAM_DEFINE_H__

/*
 * Non-volatile parameter setting
 *
 * NV_SI_*	- Parameters are SoC independent. These parameters can be
 *                used across multiple SoC and are referenced outside of the
 *                SoC specific folder.
 * NV_SS_*	- Parameters are SoC specific. These parameters are only
 *                used within the SoC specific folder.
 *
 * In a perfect design, we would split them into two files, but that will
 * leaves gap between them or more segment. In addition, one cannot see
 * them all in the same files which one find it is quite nice to see them
 * all in a single file. Unless there is a strong argument to split them,
 * they will be condensed into a single file and include by the top level
 * nvparam.h header.
 *
 * Each SoC is required to provide this file.
 */
enum nvparam {
	/*
