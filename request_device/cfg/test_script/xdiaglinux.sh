...
amp_tdp_test () {
	BM=$1
	ExCore=`cat /proc/cpuinfo |grep processor|tail -n 1|awk '{print $3}'`
	echo "----------  $1 BenchMark on $2 Cores  --------"
	echo "----------  Start time    --------"
	Core=0
	let ExCore=${ExCore}-1
	while [ ${Core} -lt $ExCore ]
	do
		taskset -c $Core ./$BM &
		let Core=${Core}+1
	done
...
for test_case in "xdiaglinux_start_test diag pci"
do
